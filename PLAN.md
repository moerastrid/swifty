...
- [ ] **Keep the hero's stats visible at all times while playing.** Today `HeroPanel` only appears inside `ConfirmPanel`, `LevelUpPanel`, and `SelectHeroPanel` — not during `DirectionPanel`/`ArtifactPanel`/`RunPanel`. **Revised decision (2026-07-14, supersedes the earlier "extend `GameView` with a `Hero` parameter" plan)**: student's own idea is the better fit — give `GamePanel` a third persistent slot (alongside `subPanel`/`CENTER` and `log`/`SOUTH`), e.g. `HeroPanel` docked at `BorderLayout.EAST`, so it's always on screen regardless of which sub-panel is active, no `GameView` interface changes needed. Concrete steps: (1) give `HeroPanel` a `refresh()` method that re-reads all `hero.getX()` values into the existing labels (currently likely only set once in the constructor — needed since `Hero` mutates in place); (2) add the `heroPanel` field + `BorderLayout.EAST` slot + `setHero(Hero)` to `GamePanel`, same style as the existing `setSubPanel(...)`; (3) call `heroPanel.refresh()` from inside `setSubPanel(...)` itself, once, so every screen-swap automatically refreshes it — no need to remember to call refresh from each individual `GuiView` method; (4) call `panel.setHero(hero)` once from `GuiView.confirmHero(Hero hero)`, the first point a `Hero` is available. Independent of the dedicated `FightSummaryPanel` item below, do either first.
