...
- [x] **`MonsterFactory` built (2026-07-07).** `mazie.model.monster.Monster` is now `abstract` (singleton `MonsterFactory` in the same package picks a random subclass per tier: `Library`/`IKEA`/`Park` easy, `Hairsalon`/`Kanteen`/`Office`/`Teams`/`Bus` medium, `Swimmingpool`/`TeamBuilding`/`Supermarket`/`Vim` hard — themed as "everyday annoyances" rather than animals). `GameMap` now owns monster generation via `factory.new{Easy,Medium,Hard}Monster(heroLevel)`; `GameEngine` only consumes `Monster`, doesn't create them anymore. Views already narrate the specific species name (`monster.getName()`) in `RunPanel`/`GamePanel`/`TerminalView` fight/run messages — nice touch, already wired up correctly.
  ...
  - **Still open — per-species stat variance doesn't actually exist yet.** Every subclass (`Library`, `Bus`, `Vim`, etc.) redeclares its own `protected static int calcAttack/calcDefence/calcHp/calcXp(lvl)` — but these are word-for-word copies of the ones already inherited from `Monster` (no per-species multiplier applied anywhere). Since Java method resolution picks the subclass's own static method over the inherited one, this compiles fine and *looks* like each monster has its own stats, but right now every monster within the same tier is still numerically identical — the entire stated motivation for this refactor (giving e.g. a tanky monster vs. a glass-cannon monster within the same tier) isn't there yet.
  - **Design agreed (2026-07-07, not yet implemented) — Template Method pattern.** `Monster`'s constructor changes shape to `(name, actions, finalMessage, lvl, baseAttack, baseDefence, baseHp, xpReward)` and does `this.attack = baseAttack + levelAttackBonus(lvl)` (same for defence/hp) — the level-scaling formulas (today's `calcAttack/calcDefence/calcHp`, renamed to something like `levelAttackBonus` etc. to make clear they're only the *level part*) move into `Monster` once, shared by everyone. Each subclass then just passes its own 3 base numbers (its "character" — tanky vs. glass-cannon) instead of recomputing full stats. Two open decisions for the student before/while implementing: (1) does `xpReward` also get a per-monster base, or stay purely level-based like now; (2) pick base values that keep the *average* roughly in line with today's implicit baseline (`17`/`2`/`20`) so overall difficulty doesn't quietly shift. Implementation is a a good next session starting point — student knows the shape, just needs to type it out subclass by subclass.
