...
- **SIGINT (Ctrl+C) handling — done 2026-07-15**: `Main.threadConfig()` also registers `Runtime.getRuntime().addShutdownHook(new Thread(...))`, capturing the same `mainThread` reference. The hook thread interrupts `mainThread` and then `mainThread.join()`s — this blocks the JVM's shutdown sequence until the main thread actually finishes unwinding (via the same `InterruptedException`→`QuitException` chain as window-close/EDT-crash), so the best-effort save still gets a chance to run before the JVM halts.
  - **Why `System.exit(...)` needed a guard**: every exit path in `main()` (all catch branches + the two calls after the try/catch) now goes through `safeExit(int exitCode)`, which gates the actual `System.exit(...)` call behind `shuttingDown.compareAndSet(false, true)`. Without this, a SIGINT-triggered shutdown (already in progress via the hook) plus the main thread's own unwind reaching its normal `System.exit(...)` call risks a genuine deadlock: `Runtime.exit()`/`Shutdown.exit()` blocks the calling thread until all registered hooks finish, while the hook itself is blocked in `mainThread.join()` waiting for that same main thread — so main must never call `System.exit()` a second time once shutdown has already begun.
  - **Why `AtomicBoolean.compareAndSet(...)` instead of a plain `volatile boolean` check-then-set**: on the interrupt-driven path there's no race (the hook writes the flag *before* calling `interrupt()`, and the JLS's interrupt happens-before guarantee means main is certain to observe it by the time it reaches `safeExit`) — but a normal, exception-free game-end reaching the tail-end `safeExit(EX_SUCCESS)` at the *exact* same moment an independent SIGINT arrives has no such ordering guarantee, so the plain-boolean read-then-write could race. `compareAndSet` makes the check-and-claim atomic, closing that gap.
  - `edtCrashed` deliberately stays a plain `volatile boolean` (not `AtomicBoolean`) — it only feeds a one-time informational read to pick an exit code, it doesn't gate a mutually-exclusive irreversible action the way `shuttingDown` does, so the worst case of a stale read is a slightly wrong exit code, never a deadlock.
...
- [ ] **Switch between console/GUI at runtime without closing.** `GameController.setView(GameView)` already exists as a hook but nothing calls it yet. Design settled 2026-07-15: (1) trigger differs per platform — `GuiView` gets a permanent button in the base `GamePanel` (not a sub-panel, so it's clickable on every screen); `TerminalView` detects a `"switch"` command the same way `scanNextLine()` already detects `"quit"`, but via a new sibling exception (not `QuitException` itself, since switching must resume the same in-progress hero/game, not end it) to unwind the blocking read. (2) Both funnel into the same `GameController.setView(...)`-driven switch logic — the GUI button via a `Runnable` callback registered on the view (`GameView.setSwitchListener(Runnable)`, Observer pattern, same idea as Swing's own `ActionListener`), the terminal via catching the new exception. (3) Because views are being made stateless (see the Swing GUI wishlist item above), a freshly-swapped-in view needs no special re-sync step — it just receives the current `Hero`/game state fresh on the next call, same as any other view.
